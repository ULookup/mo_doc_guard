...
def evaluate_quality_gate(
    *,
    ...
    if "G9" in enabled_gates and _contains_forbidden_ai_markers(doc_patch_diff):
        gate_issues.append("G9: forbidden AI marker detected in doc patch")
...
def _load_enabled_gates(quality_gates_file: Path | None) -> set[str]:
    ...
    if isinstance(gates, list):
        for gate in gates:
            ...
            if gate_id:
                enabled.add(gate_id)
    return enabled or {"G1", "G2", "G3", "G4", "G7", "G9"}


def _contains_forbidden_ai_markers(doc_patch_diff: str) -> bool:
    for line in doc_patch_diff.splitlines():
